...
export const TableView: React.FC<TableViewProps> = ({ tasks, onSelectTask }) => {
  return (
    <div className="flex-1 p-8 overflow-y-auto bg-shtab-light">
      <div className="bg-white border border-shtab-border rounded-2xl shadow-sm p-6">
        <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4">Project Timeline & Task Summary</h3>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Author: {task.author?.name || 'Unknown'} • Assigned to: {task.assignee?.name || 'Unassigned'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs font-semibold text-slate-700">
                      {hasDue ? new Date(task.due_date!).toLocaleDateString([], { month: 'short', day: 'numeric' }) : 'None'}
                    </span>
                  </div>
                </div>
              </div>
